...
def get_events(token, start_dt=None, end_dt=None):
    now = datetime.now(IST)
    if not start_dt:
    ...
    start = start_dt.replace(hour=0, minute=0, second=0, microsecond=0).astimezone(pytz.utc).isoformat()
    end = end_dt.replace(hour=23, minute=59, second=59, microsecond=0).astimezone(pytz.utc).isoformat()
...
def delete_meeting(token, match_term):
    start_range = datetime.now(IST) - timedelta(days=7)
    end_range = datetime.now(IST) + timedelta(days=30)
    events = get_events(token, start_range, end_range)
    ...
    if response.status_code == 204:
        ...
        return False, "Permission denied. Ensure app has `Calendars.ReadWrite` permission."
    elif response.status_code == 404:
        return False, "Event not found (already deleted or inaccessible)."
    else:
